...
std::string cKinTree::BuildJointMatJson(const Eigen::MatrixXd& joint_mat, cBVHReader *r)
{
	std::string json = "";
	json += "{\n\"" + gJointsKey + "\":\n[\n";
	...
	for (int j = 0; j < num_joints; ++j)
	{
		...
		std::string joint_json = BuildJointJson(j, curr_desc, r);
...
std::string cKinTree::BuildJointJson(int id, const tJointDesc& joint_desc, cBVHReader *r)
{
        cBVHReader::tJointData *joint = 0;
        if( joint_counter == id)
            joint = (cBVHReader::tJointData *)&r->GetJointData(id);
        else
            joint = (cBVHReader::tJointData *)&r->GetJointData(joint_counter);
	...
	json += "\"ID\": " + std::to_string(id);
        ...
        if(!joint->IsValid()) 
            joint = (cBVHReader::tJointData *)&r->GetJointData(++joint_counter);
//        std::cout << "ID="<< std::to_string(id) << " JOINT NAME=" << joint->name << " VALID=" << joint->IsValid() <<"\n";
        joint_counter++;
//        if(!joint.IsValid()) {
//             const cBVHReader::tJointData& joint1 = r->GetJointData(id + 1);
//        
//             std::cout << "ID1="<< std::to_string(id + 1) << " JOINT NAME=" << joint1.name << " VALID=" << joint1.IsValid() <<"\n";
//        } else {
//            std::cout << "ID="<< std::to_string(id) << " JOINT NAME=" << joint.name << " VALID=" << joint.IsValid() <<"\n";
//        }
